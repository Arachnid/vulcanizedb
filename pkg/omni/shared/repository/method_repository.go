...
func (r *methodRepository) PersistResults(results []types.Result, methodInfo types.Method, contractAddr, contractName string) error {
	if results == nil {
		return errors.New("method repository error: passed nil results")
	}
	if len(results) == 0 {
		return nil
	}
...
func (r *methodRepository) persistResults(results []types.Result, methodInfo types.Method, contractAddr, contractName string) error {
	...
	methodId := strings.ToLower(methodInfo.Name + "_" + contractAddr)
	err = repository.MarkHeaderCheckedInTransaction(results[0].Block, tx, methodId) // This assumes all logs are from same block
	if err != nil {
		tx.Rollback()
		return err
	}
